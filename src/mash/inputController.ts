...
const { ActionManager, ExecuteCodeAction, Scalar } = BABYLON;
...
export default class PlayerInput {
  ...
  private _updateFromKeyboard(): void {
    ...
    if ((this.inputMap[UP] || this.mobileUp) && !this._ui.gamePaused) {
      this.verticalAxis = 1;
      this.vertical = Scalar.Lerp(this.vertical, 1, 0.2);
    } else if (
      (this.inputMap[DOWN] || this.mobileDown) &&
      !this._ui.gamePaused
    ) {
      this.vertical = Scalar.Lerp(this.vertical, -1, 0.2);
      this.verticalAxis = -1;
    ...
    //left - right movement
    if ((this.inputMap[LEFT] || this.mobileLeft) && !this._ui.gamePaused) {
      ...
      (this.inputMap[RIGHT] || this.mobileRight) &&
      !this._ui.gamePaused
    ) {
      this.horizontal = Scalar.Lerp(this.horizontal, 1, 0.2);
      this.horizontalAxis = 1;
